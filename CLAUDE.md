...
- **Présentation 3D** : `qml/world3d/World3D.qml` (View3D + helpers grid↔world, tick batch `bodyStates(ids)` par FrameAnimation), `PhysicsActor.qml` (présentateur 3D, `applyState` avec lissage framerate-indépendant, **Y visuel 2.5D** via `visualY` + `jump()`/`wave()`), `CrateSpawner.qml`/`EnemySpawner.qml` (spawn auto sur tiles `PhysicalObjectTile`/`EnemyTile`), `LocalPlayerSpawner.qml`, `EditorPhysicsBridge.qml`, `InputController.qml` (clavier → `pushInput(actorId, vec)`), `CameraRig.qml` (modes `Follow`/`Free`/`FixedTopDown`), contrôleurs gameplay (`CombatController`, `GrabController`, `TriggerController`), `BodyIds.js` (préfixes de bodyId partagés).
...
- `Meownopoly/qml/` — QML UI (editor, board, chat, menu, launcher, account, components)
  - `qml/world3d/` — Présentation 3D physique (World3D, PhysicsActor, CrateSpawner, EnemySpawner, LocalPlayerSpawner, EditorPhysicsBridge, InputController, CameraRig, CombatController, GrabController, TriggerController, BodyIds.js)
...
- **Passerelle MCP du canal IA (AiGatewayServer)** : depuis 2026-07-18, **toujours active par défaut** (`MEOW_AI_GATEWAY_ALWAYS_ON=1`, compile-time) sur **7790** (+ instance-1 en dual-instance → 7790/7791), 127.0.0.1 + Bearer token obligatoires. Port explicite via `--ai-gateway-port <N>` / `MEOW_AI_GATEWAY_PORT` ; la valeur **0** est l'opt-out runtime. L'add-on **QtHttpServer est installé sur le kit** (`MEOW_HAS_HTTP_SERVER=1` — si le configure dit « absent », c'est un cache/PATH stale : reconfigurer avec le PATH MinGW). Tokens de session lisibles dans l'onglet V3 IA (V3GatewayPanel, via `AiProcessSupervisor.gatewayProposerToken()`).
- **Banc d'essai `meow_testbench`** : construit automatiquement avec le jeu (`add_dependencies(Meownopoly meow_testbench)`) — plus besoin de la cible manuelle. `BenchSupervisor` résout l'exe via `MEOW_BENCH_EXE` (env), puis à côté du jeu, puis dans les sous-dossiers de config (Release/Debug…). **Journal de diagnostic du pipeline banc** : `%TEMP%\meow_bench_debug.log` (résolution du chemin, spawn, exitCode, stdout/stderr, verdict) — à lire en premier quand « Valider au banc » rend un FAIL environnemental. Piège corrigé : `finishWith` émet le verdict en `QueuedConnection` (une émission synchrone pré-spawn arrivait avant que le QML ait stocké le jobId → verdict perdu, bouton bloqué sur « Validation… »).
