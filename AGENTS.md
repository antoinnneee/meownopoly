...
- **Codex `exec` non interactif exige les annotations MCP des tools** : chaque descripteur de `tools/list` doit porter des valeurs exactes pour `readOnlyHint`, `destructiveHint`, `idempotentHint` et `openWorldHint`. Sans elles, Codex 0.144.5 peut demander une approbation impossible à afficher avec `approval: never`, puis annuler localement avant tout `tools/call` avec `user cancelled MCP tool call`. Source runtime : `AiGatewayServer::toolTable/toolDescriptor` ; source contractuelle : `channel_manifest.json`.
