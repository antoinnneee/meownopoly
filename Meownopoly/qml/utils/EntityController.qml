...
Item {
     ...
     * Vérifie plusieurs points le long du rayon de collision
     */
    function canMoveToGridPos(newX, newZ) {
        // Vérifier le centre
        if (isInExclusionZone(newX, newZ)) return false
    ...
    FrameAnimation {
        ...
        onTriggered: {
            ...
            if (inputVector.length() > 0) {
                ...
                // Mode FreeCam : On déplace la caméra directement
                if (root.freeCamMode) {
                    // On utilise le mouvement manuel du CameraController
                    // inputVector.x -> axe X (droite/gauche)
                    // inputVector.y -> axe Z (haut/bas de l'écran -> avant/arrière en 3D)
                    // On peut augmenter la vitesse en freecam si besoin (ex: * 2.0)
                    CameraController.moveManual(inputVector.x, inputVector.y, speed * 1.5, dt)
                } 
                // Mode Personnage : On déplace l'entité
                else {
