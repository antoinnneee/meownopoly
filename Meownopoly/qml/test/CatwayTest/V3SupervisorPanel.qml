...
Rectangle {
    ...
    // Rôle ciblé par les contrôles de configuration (0 = Proposante, 1 = Arbitre).
    property int _role: AiProcessSupervisor.Proposer
    ...
    function _buildOpts() {
        ...
        if (modelField.text.trim().length > 0)
            opts["model"] = modelField.text.trim()
        return opts
    }
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            FieldLabel { text: "Rôle" }
            ComboBox {
                id: roleCombo
                objectName: "supRoleCombo"
                Layout.fillWidth: true
                implicitHeight: Theme.px(28)
                font.pixelSize: Theme.fontSizeSmall
                model: ["Proposante", "Arbitre"]
                currentIndex: 0
                onCurrentIndexChanged: {
                    root._role = currentIndex === 1 ? AiProcessSupervisor.Arbiter
            ...
            StyledField {
                id: programField
                objectName: "supProgramField"
                Layout.fillWidth: true
                text: adapterCombo.currentIndex === 1 ? "codex" : "claude"
            ...
            StyledField {
                id: modelField
                objectName: "supModelField"
                Layout.fillWidth: true
                placeholderText: "(optionnel) ex. claude-opus-4"
            }

            FieldLabel { text: "Prompt initial" }
            StyledField {
                id: promptField
                objectName: "supPromptField"
                Layout.fillWidth: true
        ...
        RowLayout {
            ...
            StyledButton {
                objectName: "supStartBtn"
                text: "Start"
                tint: Theme.success
                onClicked: AiProcessSupervisor.startAgent(root._role, root._buildOpts())
            }
            StyledButton {
                objectName: "supStopBtn"
                text: "Stop"
                onClicked: AiProcessSupervisor.stopAgent(root._role)
            }
            StyledButton {
                objectName: "supRestartBtn"
                text: "Restart"
                onClicked: AiProcessSupervisor.restartAgent(root._role)
            }
            Item { Layout.fillWidth: true }
            StyledButton {
                objectName: "supTestArbiterBtn"
                text: "Tester l'arbitre"
                tint: root.host.accent
                onClicked: AiProcessSupervisor.testArbiter(root._buildOpts())
            }
        }
        ...
        GridLayout {
            ...
            FieldLabel { text: "proposerState" }
            Text {
                objectName: "supProposerState"
                text: root._stateName(AiProcessSupervisor.proposerState)
                color: root._stateColor(AiProcessSupervisor.proposerState)
                font.pixelSize: Theme.fontSizeSmall; font.family: "Consolas"; font.bold: true
            }
            FieldLabel { text: "arbiterState" }
            Text {
                objectName: "supArbiterState"
                text: root._stateName(AiProcessSupervisor.arbiterState)
                color: root._stateColor(AiProcessSupervisor.arbiterState)
                font.pixelSize: Theme.fontSizeSmall; font.family: "Consolas"; font.bold: true
            }
            FieldLabel { text: "arbiterReady" }
            Text {
                text: "" + AiProcessSupervisor.arbiterReady
                color: AiProcessSupervisor.arbiterReady ? Theme.success : root.host.textSecondary
                font.pixelSize: Theme.fontSizeSmall; font.family: "Consolas"
            }
            FieldLabel { text: "handshakeReason" }
            Text {
                objectName: "supHandshakeReason"
                Layout.fillWidth: true
        ...
        // — Indicateur lobby 4-états (arbitre, D31) —
        RowLayout {
            Layout.fillWidth: true
            spacing: Theme.spacingM
            FieldLabel { text: "Lobby arbitre" }
            Rectangle {
                objectName: "supLobbyBadge"
        ...
        // — Sortie récente (bornée, scrollable, monospace) —
