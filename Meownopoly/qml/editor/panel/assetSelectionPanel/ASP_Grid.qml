...
ScrollView {
    ...
    function updateModel() {
        if (category && type) {
            assetModel = AssetManager.getTypeModel(category, type)
        } else {
            assetModel = null
        }
    }
    ...
    Rectangle {
        ...
        height: 100
        color: "transparent"
        visible: !root.assetModel && root.category && root.type
    ...
    Rectangle {
        ...
        color: "transparent"
        visible: {
            if (!root.assetModel || root.searchText === "") return false
            
            // Count visible items
            var visibleCount = 0
            for (var i = 0; i < root.assetModel.rowCount(); i++) {
                var searchLower = root.searchText.toLowerCase()
                var item = root.assetModel.data(root.assetModel.index(i, 0))
                if (item) {
                    var idMatch = (item.id || "").toString().toLowerCase().includes(searchLower)
                    var filenameMatch = (item.filename || "").toLowerCase().includes(searchLower)
            ...
            return visibleCount === 0 && root.assetModel.rowCount() > 0
        }
