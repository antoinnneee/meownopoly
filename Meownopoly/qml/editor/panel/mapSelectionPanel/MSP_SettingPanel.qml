...
Rectangle {
    ...
    // Initialisation optimisée pour mesurer les hauteurs
    Timer {
        id: heightUpdateTimer
        interval: 100 // Petit délai pour s'assurer que tout est rendu
        running: false
        repeat: true
        ...
        onTriggered: {
            ...
            // Vérifier si on a mesuré toutes les vues
            var allMeasured = viewHeights.general > 0 && 
                            viewHeights.saveLoad > 0 && 
                            viewHeights.background > 0;
            ...
            if (allMeasured) {
                ...
                // Essayer de mesurer d'autres vues en changeant la vue courante temporairement
                // MAIS sans toucher aux bindings de visibilité
                if (contentArea.currentView !== "general" && viewHeights.general <= 0) {
                    var currentView = contentArea.currentView;
                    contentArea.currentView = "general";
                    Qt.callLater(function() { contentArea.currentView = currentView; });
                } 
                else if (contentArea.currentView !== "saveLoad" && viewHeights.saveLoad <= 0) {
                    var currentView = contentArea.currentView;
                    contentArea.currentView = "saveLoad";
                    Qt.callLater(function() { contentArea.currentView = currentView; });
                }
                else if (contentArea.currentView !== "background" && viewHeights.background <= 0) {
                    var currentView = contentArea.currentView;
    ...
    Component.onCompleted: {
        // Mesurer la vue initiale
        Qt.callLater(function() {
            measureCurrentView();
            
            // Lancer le timer pour mesurer toutes les vues
