...
- [ ] 🟡 **M7 — Mouvement restant après impact jeté** — `pattounx_engine_v2.cpp:324-345`. Après rewind à `t`, pas de ré-intégration sur `(1-t)*dt` → murs "collants", slide oblique ralenti. Fix : `position += velocity * (1-t)*dt` + re-sweep (1-2 itérations).
- [ ] 🟡 **M8 — Body endormi exclu de la détection zone** — `pattounx_engine_v2.cpp:352-374`. Zone draguée sur une caisse endormie = caisse enfouie jusqu'à réveil externe. Fix : réveiller les bodies dans l'AABB d'une zone sur `upsertZone`/`removeZone`.
- [ ] 🟡 **M9 — `sweepCircleVertex` incohérent avec `sweepCircleCircle` sur départ en pénétration** — `collision2d.cpp:287-310`. Si déjà dans le disque du sommet, aucun contact (au lieu de `t=0`). Fix : si `c <= 0`, retourner `t=0` avec normale `startPos - vertex`.
- [ ] 🟡 **M10 — Cap de vitesse max inopérant si `linearDamping == 0`** — `pattounx_engine_v2.cpp:251-257`. `decel == 1` → un body sans damping recevant une grosse impulsion dépasse `maxSpeed` indéfiniment. Fix : decel minimal garanti dans la branche cap, ou clamp dur à 2× maxSpeed.
- [ ] 🟡 **M11 — `ShapeType::Polygon` = API morte** — `pattounx_types.h:27-30` + `pattounx_engine_v2.cpp:283/390-391`. Un body polygone ne collisionne avec rien, silencieusement. Fix : rejeter/logguer à l'`upsertBody` (ou le supporter).
- [ ] 🟡 **M12 — Friction de zone par défaut 0.5 hardcodée** — `pattounx_engine_v2.cpp:543-547`. `if (zf > 0.0)` empêche d'exprimer une friction nulle (zone glissante) — `frictionStrength=0` est une valeur légitime. Fix : utiliser la valeur clampée ≥ 0, réserver le défaut au cas "zone introuvable".
- [ ] 🟡 **M13 — `isColliding` incohérent** — `pattounx_engine_v2.cpp:337/503-508`. Ne reflète que body-zone, jamais body-body ; reset seulement dans une branche. Fix : reset en début de step, set dans les deux résolutions.
- [ ] 🟡 **M14 — `removeBody` n'émet pas les `exited` des zones actives** — `pattounx_engine_v2.cpp:46-50`. Les consommateurs à pile de zones (QML) gardent un état orphelin. Fix : émettre `exited` pour chaque zone de `m_activeZonesPerBody[id]` avant remove.
- [ ] 🟡 **M15 — `upsertBody` : sémantique d'update non documentée** — `pattounx_engine_v2.cpp:31-44`. `spec.position` ignorée sur body existant (voulu, non documenté) ; changement de radius ne réveille pas le body. Fix : documenter dans le .h + `wakeUp` sur changement de shape.
- [ ] 🟡 **M16 — `EPSILON` unique pour des grandeurs hétérogènes** — `collision2d.h:214`. Longueurs, produits scalaires, longueurs² partagent 1e-4. Fix : documenter l'unité de référence ou epsilons dédiés.
- [ ] 🟡 **M17 — Nettoyages** : clamp friction statique `<` → `<=` (`collision2d.cpp:489`) ; bbox `QRectF::contains` exclut bords droit/bas (`collision2d.cpp:184`, gonfler d'un epsilon).
