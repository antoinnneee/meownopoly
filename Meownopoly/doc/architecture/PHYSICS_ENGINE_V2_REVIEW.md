...
- [x] 🟡 **N10 — Retour en sim locale après perte d'hôte = téléportation** *(corrigé 2026-07-03 : `setUseRemoteBuffer(false)` réinjecte les positions du remote buffer via `cmdSetBodyPosition` (no-op pour les bodies inexistants localement, sync `previousPosition` donc pas de faux sweep) AVANT `resetNetworkState`)* — `physics_session.cpp:433-439`. `stop()` client purge le remote buffer puis reprend la sim locale sur l'état d'avant-session. Fix : réinjecter le dernier snapshot dans le moteur local avant purge (ou documenter que l'appelant re-spawne).
...
- [x] 🟡 **N14 — Pas de versionning de protocole** *(corrigé 2026-07-03 : format `[type][kProtocolVersion=1][payload]`, vérification dans `peekType` (chokepoint de toute réception) avec warning throttlé ; `payloadBytes`/`unpackJson` décalés)* — `physics_protocol.cpp:5-25`. Deux builds au format différent désérialisent du garbage silencieusement. Fix : octet `kProtocolVersion` après le type byte, rejet avec warning si mismatch.
- [x] 🟡 **N15 — Wrap de `m_nextIdIndex` (quint16)** *(corrigé 2026-07-03 : free-list FIFO `m_freeIdIndices` alimentée à la disparition des bodies, recyclée à l'allocation — le plus ancien libéré d'abord, la full table 1 Hz (N9) resynchronise les clients ; garde qCritical si 65535 bodies vivants simultanés)* — `physics_world.cpp:440`. Jamais recyclé : wrap vers 0 (sentinel !) après 65535 spawns cumulés puis collisions d'index. Fix : free-list des index libérés, ou quint32 + resync forcé au wrap.
...
- [x] 🟡 **N17 — `pushOrSendInput` : un paquet reliable par appel** *(corrigé 2026-07-03 : dédup par actorId via `m_lastSentInputs`, purgé au stop ET à chaque Hello — les inputs pré-claim rejetés par l'hôte doivent pouvoir repartir même inchangés)* — `physics_session.cpp:229-239`. Si appelé chaque frame, 60-144 pkts/s pour un vecteur quasi constant. Fix : n'envoyer que si `input != m_lastSentInput`.
- [x] 🟡 **N18 — Nits** *(corrigé 2026-07-03 : `qRound(1000.0/hz)` aux deux sites ; la comparaison wraparound-aware était déjà documentée sur `m_lastRemoteTick` depuis le fix N6)* : `1000 / m_snapshotHz` en division entière (`physics_session.cpp:55,94`, → `qRound(1000.0/hz)`) ; documenter la comparaison wraparound-aware du tick quint32 dans le header.
...
- [x] 🟡 **Q7 — Fantôme à l'origine côté client avant le premier snapshot** *(corrigé 2026-07-03 : `seeded` public sur PhysicsActor + `reseed()` (API publique, remplace les `_seeded = false` cross-privés) + dé-seed après `missTolerance` (90 frames ≈ 1,5 s) d'échecs consécutifs ; `visible` des deux spawners conditionné sur `actor.seeded` côté client)* — `CrateSpawner.qml:144-146`, `EnemySpawner.qml:149-151`. `visible: running` mais `bodyState` échoue tant que le BodiesAnnounce n'est pas arrivé → node à (0,0,0) ; body retiré côté hôte = node figé (rien ne couvre les caisses). Fix : exposer `seeded` sur PhysicsActor et conditionner `visible` dessus ; dé-seeder après N échecs de `bodyState`.
...
- [x] 🟡 **Q9 — Orientation `atan2(vx, vy)` incohérente avec le mapping `Z3D = -gy`** *(statué 2026-07-03 : documenté dans PhysicsActor — le miroir est compensé par l'orientation de base des modèles (validé visuellement, les actors regardent dans leur direction de déplacement) ; ne pas "corriger" sans re-tester tous les modèles)* — `PhysicsActor.qml:156` vs `World3D.qml:52`. Yaw en miroir sur Z, peut-être compensé par le modèle — vérifier visuellement, corriger ou documenter.
- [x] 🟡 **Q10 — `enemyParameter` déréférencé sans garde** *(corrigé 2026-07-03 : `if (!ep) continue` dans `_aiTick` + garde dans `_ensureState`)* — `CombatController.qml:164-166`. TypeError au milieu de `_aiTick` si tile Enemy sans paramètre (partout ailleurs il est traité nullable).
- [x] 🟡 **Q11 — Caisse déplacée dans l'éditeur moteur tournant : body jamais repositionné** *(corrigé 2026-07-03 : Connections `ItemSnapableEvents.tileMoved` dans le delegate de CrateSpawner → `setBodyPosition(bodyId, _spawnPos())` (sync `previousPosition`, pas de faux sweep) quand le body est spawné)* — contrairement aux zones (bridge `tileMoved`). Documenter le choix, ou écouter `tileMoved` et `removeBody`+`create` (ou commande `teleportBody`).
- [x] 🟡 **Q12 — Divers** *(corrigé 2026-07-03 : `upsertZoneNow` promu API publique du bridge + commandes moteur de `_applyTargets` gatées sur `isAuthority` (le visuel reste pour tous) ; `el.opacity` impératif documenté comme assumé (runtime only, restauré, aucun binding existant) ; `keysHandler` supprimé d'InputController (code mort — celui d'EditorController est un autre composant, réellement utilisé) ; LocalPlayerSpawner documenté (le gate `running` est voulu : worker mort = plus de body à retirer ; bodyId global = partagé entre scènes simultanées)*

### Performance

- [x] 🟡 **Q13 — Pull pattern : N appels `bodyState()` QML→C++ par frame, 1 QVariantMap chacun** *(corrigé 2026-07-03, avec T13 : le tick de World3D collecte les bodyIds du registry et fait UN appel `bodyStates(ids)` (liste alignée), chaque actor reçoit son état via `applyState(s, dt)` — `pullAndApply` reste comme wrapper de compat)*
...
- [x] 🟡 **Q14 — 7 composants refiltrent `snapableTilesList` à chaque `tilesRevision`** *(statué 2026-07-03 : non retenu — les filtres ne tournent qu'à `tilesRevision` (mutation d'édition), jamais par frame ; le gain ne justifie pas de toucher l'interface de 7 composants. À revisiter si les maps deviennent très grandes)* — `Editor.qml:1527-1611`. Fix : un "TileIndex" partagé qui classe par tileType une fois par revision.
- [x] 🟡 **Q15 — Recherches linéaires par uuid dans les tick-paths** *(corrigé 2026-07-03 : `_crateTileByUuid` dans GrabController (tick maintien 30 Hz), `_tilesByUuidMap`/`_triggerTileByUuidMap` dans TriggerController (events enter/exit) ; `_tileByUuid` de CombatController supprimé — code mort sans appelant)* — `CombatController.qml:139-146`, `GrabController.qml:210-218`. Fix : map `{uuid: tile}` reconstruit avec les listes filtrées.
- [x] 🟡 **Q16 — ~90 lignes d'instrumentation jitter dans le présentateur de prod** *(corrigé 2026-07-03 : instrumentation supprimée (git garde l'historique, le bug 144 Hz est résolu et documenté en mémoire projet) + raccourci J retiré d'Editor.qml)* — `PhysicsActor.qml:53-123`. Le bug 144 Hz est résolu : extraire dans un composant debug attachable.

### Architecture

- [x] 🟠 **Q17 — Duplication CrateSpawner/EnemySpawner (~80 % de squelette commun)** *(corrigé partiellement 2026-07-03 : `_bodySpec()` unique + properties `enemyBodyRadius`/`enemyAcceleration`/`enemyLinearDamping` dans EnemySpawner — la spec dupliquée intra-fichier (le vrai piège de divergence) est éliminée. Le composant `BodyLifecycle` commun reste une dette de refactor, à faire avec un vrai passage de test des deux spawners)*
...
- [x] 🟡 **Q18 — Préfixes de bodyId éparpillés** *(corrigé 2026-07-03 : module `qml/world3d/BodyIds.js` (.pragma library — crate/enemy/isCrate/isEnemy/crateUuid/enemyUuid), consommé par CrateSpawner, GrabController, CombatController et TriggerController ; ajouté au qml.qrc)* — `"crate:"` dans CrateSpawner:48 et GrabController:69, `substring(6)` magique dans TriggerController:113-119. Fix : module JS partagé (`BodyIds.crate(uuid)`, `BodyIds.isCrate(id)`).
- [x] 🟡 **Q19 — Gameplay dans le dossier présentation + appels cross-privés** *(corrigé 2026-07-03 : `applyLoot` public sur CombatController, `reseed()` public sur PhysicsActor (avec Q7) — plus d'accès cross-privés. Le déménagement vers `qml/gameplay/` reste "à terme")* — `combat._applyLoot` (TriggerController:218), `actor._seeded = false` (spawners). Fix : promouvoir `applyLoot`/`reseed()` en API publiques ; viser `qml/gameplay/` à terme.
- [x] 🟡 **Q20 — CombatController force `enabled = true` sur les modules gameplay** *(statué 2026-07-03 : documenté comme choix assumé du mode test éditeur (un event de combat prouve l'usage de la feature) avec pointeur vers le déplacement futur dans le câblage du GameplayModuleManager si gênant)* — `:203,296,416,420`. Le GameplayModuleManager est censé posséder l'activation ; ré-activer un module que l'utilisateur vient de couper est surprenant. Déplacer vers le câblage du manager ou documenter.
- [x] 🟡 **Q21 — Constantes magiques à promouvoir en properties** *(corrigé 2026-07-03 : `springStiffness`/`springDamping`/`maxImpulsePerMass` (GrabController), `enemyBodyRadius`/`enemyAcceleration`/`enemyLinearDamping` (EnemySpawner, avec Q17), `groundRefDistance` (CameraRig), `aiTickIntervalMs` (CombatController))* — ressort du grab (`GrabController.qml:201-205`), radius/accel ennemi, `cam.z - 600`, tick IA 100 ms.
- [x] 🟡 **Q22 — Doc : CLAUDE.md liste encore `PhysicsObjectSpawner.qml`** *(corrigé 2026-07-03 : les deux mentions remplacées par CrateSpawner/EnemySpawner + contrôleurs gameplay + BodyIds.js)* (absorbé par CrateSpawner) — corriger la dérive.
