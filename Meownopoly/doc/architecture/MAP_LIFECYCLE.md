...
| #     | Localisation                                                       | Problème                                                                                                                     | Impact / décision                                |
|-------|--------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|
| **G1** | `mapfilemanager.cpp:276` (`createMapFile`)                        | écrit `mapInfo["mapName"] = mapName` mais `MapInfo::MapInfo(json)` lit `json["name"]`                                       | Le fichier vide créé a un mapName **non lisible**. Masqué par les call sites qui font `mapInfo.mapName = X` après `loadMap`. **À fixer.** |
| **G2** | `mapinfo.cpp:107,113,198,204` (`setMapName/Description/LastModified/Version`) | pas de guard d'égalité avant `emit` (contrairement aux autres setters)                                              | Risque de boucle de binding (cf. commit `0e1a16c`). **À fixer.** |
| **G3** | `mapfilemanager.cpp:191-213` (`renameMap`)                         | `QFile newMap(oldMapName)` utilise des noms bruts (pas de chemin) ; `flag` reste `false` même en succès                     | Code mort/cassé. **À supprimer** (jamais appelé en QML). |
