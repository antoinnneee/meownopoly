...
> chantier M2 (doc 10) mais sans document dédié jusqu'ici — le doc 01 §2.7 la
...
V3 : c'est lui qui matérialise « son IA » (doc 00 §2). Il couvre deux
...
2. **Tchat ingame** : l'UI de dialogue joueur↔IA depuis laquelle chaque
   ...
   explications de rejet (S3 du slice, doc 11).
...
### 2.4 Tchat ingame
- Une invocation = un tour de tchat (modèle d'événements doc 02 §4).
- Affiche : réponses de l'IA, verdicts (`reasons[audience=player]`, doc 13 §4),
  progression des propositions, erreurs actionnables.
- Information « captures d'écran actives » affichée une fois au lancement du
  mode IA (D22) ; de même pour le prérequis compte + CLI (doc 00 §8).
- Budget/coût : l'hôte configure un budget (D10) ; les joueurs configurent
  ...
  « objectifs propres » à cadrer, doc 00 §4 note, doc 08 §2).
...
- **Socle repris** : `LauncherManager` comme patron du cycle d'opérations
  ...
  aujourd'hui (doc 10, B03) : la supervision est un chantier neuf.
- **Frontières** : l'adaptateur ne parle pas MCP (c'est le canal, doc 02) et
...
- **Onboarding guidé** : détection des CLIs installés, aide à la connexion au
  compte fournisseur (prérequis doc 00 §8) — périmètre exact à cadrer.
...
- **Multi-invocations** : une invocation à la fois par rôle (file), ou
  proposante et arbitre en parallèle ? (Le pipeline doc 13 sérialise déjà les
