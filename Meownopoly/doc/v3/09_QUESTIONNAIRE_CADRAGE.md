...
- **Réponse : TRANCHÉE (Antoine, 2026-07-17) → décision D44.** Mécanique
validée telle que proposée (curseur monotone, résumé injecté par invocation,
`{truncated, oldestSeq}`), avec les trois sous-points arbitrés :
(1) plafond du bloc injecté **amendé à 250 lignes** (au lieu des 30 proposées) ;
(2) types « pertinents » = la recommandation (`tile.placed`, `proposal.verdict`,
`memory.changed` publics, changements de règlement) ; (3) les verdicts de
**dry-run n'entrent pas** au journal partagé — confirmé. Acté dans le
**manifeste du canal** [`cpp/ai/gateway/channel_manifest.json`](../../cpp/ai/gateway/channel_manifest.json)
(bloc `eventSummary`, `protocolVersion 1.0.0`).
...
- **Sous-ensemble retenu : TRANCHÉ (Antoine, 2026-07-17) → décision D43.**
Les **12 tools validés tels quels** (liste et regroupements ci-dessus ;
`runtime_input`/`save_map`/tools caméra restent post-MVP). Répartition par rôle
**confirmée** : `arbiter_verdict` arbitre uniquement ; l'arbitre a les seuls
tools de lecture (`help`, `state_query`, `events_poll`, `screenshot`) + son
verdict ; le proposant a tout le reste. **Quotas/rate-limits : valeurs de
départ actées** dans le bloc `quotas` du manifeste (à recaler avec les mesures
Q-J04 dès le vertical slice) ; la granularité du groupement reste à mesurer
sur les premiers workflows sans bloquer l'implémentation. Le manifeste
[`cpp/ai/gateway/channel_manifest.json`](../../cpp/ai/gateway/channel_manifest.json)
(`protocolVersion 1.0.0`) est désormais la **source de vérité unique** de
M1 (catalogue, C3) et M12 (skill, C8).
