...
| C3 · Tools MVP (manifeste P0-4) | 12 tools Q-E08 ; traduction vers les hooks existants (`editorAutomationHooks`, `Editor.qml:2634-3118`) via le chemin UI exact (`placeSelectedAsset`+`Game.updateMap`, compatible collab/undo). Descripteurs enrichis des annotations MCP de sécurité requises par Codex non interactif ; `state_query` expose aussi `asset_categories`/`assets` pour découvrir les ids avant pose | **É** |
...
| C5 · `AiProcessSupervisor` (M2, [doc 14](./14_ADAPTATEUR_AGENTS.md)) — **implémenté** | `cpp/ai/ai_process_supervisor.{h,cpp}` : `QProcess`, adaptateurs `claude -p`/Codex séparés, start/stop gracieux/kill/timeout/crash/restart, stdout/stderr bornés, secrets hors argv, Job Object Windows anti-orphelin. États QML `Stopped/Starting/Ready/Failed/Restarting` | **É** |
| C6 · Handshake + challenge arbitre (D24/D31) — **implémenté** | Vérification de rôle/capacité avant ouverture du mode IA ; lobby 4 états, configuration des modèles, logs de diagnostic, bouton grisé hors `Prêt`, re-test manuel/auto ; création directe d'une session `[AI-EDIT:]` puis entrée éditeur | **M** |
| C7 · Tchat ingame IA — **implémenté (UI + invocation)** | `AiChatDrawer`, distinct du chat multijoueur : ouverture automatique dans l'éditeur IA, boutons classique/nouveau chrome, une invocation proposant par tour, réponses/verdicts/progression et info captures D22. La réponse joueur est isolée sur stdout ; stderr reste dans les diagnostics du superviseur. L'orchestration complète proposant→arbitre reste portée par S-1/S-2. | **É** |
