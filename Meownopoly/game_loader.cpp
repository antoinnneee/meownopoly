...
void Game::init_caseFile() {
  ...
  while (line < caseInfo.count())
  {
      ...
      switch (type)
      {
          ...
          {
              int reward = 200;
              CaseKibbleDispenser* kibbleCase = new CaseKibbleDispenser(name,
  position, reward, this); boardCase = kibbleCase; if (DEBUG_CASE_LOADING) {
                  qDebug() << "Created Kibble Dispenser" << name << "at
  position" << position;
              }
              break;
          }
          case CT_RestArea: // Properties
          {
              ...
              for (int i = 0; i < CASE_PRICE_COUNT; i++) {
                  if (CASE_PRICE_COLUMN + i < caseInfoLine.size() &&
  !caseInfoLine[CASE_PRICE_COLUMN + i].isEmpty()) {
                      prices.append(caseInfoLine[CASE_PRICE_COLUMN +
  i].toInt()); } else { prices.append(0);
                  }
              }

              FamilyType family = FT_NONE;
              if (CASE_FAMILY_COLUMN < caseInfoLine.size() &&
  !caseInfoLine[CASE_FAMILY_COLUMN].isEmpty()) { family =
  static_cast<FamilyType>(caseInfoLine[CASE_FAMILY_COLUMN].toInt());
              }

              CaseRestArea* restCase = new CaseRestArea(name, prices, family,
  position, this); boardCase = restCase; if (DEBUG_CASE_LOADING) { qDebug() <<
  "Created Rest Area" << name << "at position" << position << "with family" <<
  family; qDebug() << "    Prices:" << prices;
              }
              break;
          }
          case CT_CardBoardBox: // Community Chest
          {
              CaseCardBoardBox* cardBoardCase = new CaseCardBoardBox(name,
  position); boardCase = cardBoardCase; if (DEBUG_CASE_LOADING) { qDebug() <<
  "Created Card Board Box" << name << "at position" << position;
              }
              break;
          }
          case CT_CatNip: // Chance
          {
              ...
              if (DEBUG_CASE_LOADING) {
                  qDebug() << "Created Cat Nip" << name << "at position" <<
  position;
              }
              break;
          }
          case CT_Jail: // Jail
          {
              ...
              if (DEBUG_CASE_LOADING) {
                  qDebug() << "Created Jail" << name << "at position" <<
  position;
              }
              break;
          }
          case CT_ToJail: // Go to Jail
          {
              ...
              if (DEBUG_CASE_LOADING) {
                  qDebug() << "Created Go To Jail" << name << "at position" <<
  position;
              }
              break;
          }
          ...
          {
              int baseRent = 25;
              if (CASE_PRICE_COLUMN < caseInfoLine.size() &&
  !caseInfoLine[CASE_PRICE_COLUMN].isEmpty()) { baseRent =
  caseInfoLine[CASE_PRICE_COLUMN].toInt();
              }

              CaseCatDoor* catDoorCase = new CaseCatDoor(name, position,
  baseRent); boardCase = catDoorCase; if (DEBUG_CASE_LOADING) { qDebug() <<
  "Created Cat Door" << name << "at position" << position << "with rent" <<
  baseRent;
              }
              break;
          }
          case CT_FreeNap: // Free Parking
          {
              ...
              if (DEBUG_CASE_LOADING) {
                  qDebug() << "Created Free Nap" << name << "at position" <<
  position;
              }
              break;
          }
  ...
  caseInfoLine[CASE_PRICE_COLUMN].toInt();
              ...
              CaseCatDoor* utilityCase = new CaseCatDoor(name, position,
  baseRent); boardCase = utilityCase; if (DEBUG_CASE_LOADING) { qDebug() <<
  "Created Utility" << name << "at position" << position << "with rent" <<
  baseRent;
              }
              break;
          }
          ...
          case CT_FurTax: // Income Tax
          {
              int taxAmount = 0;
              if (CASE_PRICE_COLUMN < caseInfoLine.size() &&
  !caseInfoLine[CASE_PRICE_COLUMN].isEmpty()) { taxAmount =
  caseInfoLine[CASE_PRICE_COLUMN].toInt();
              }

              // For now, treat taxes as kibble dispensers with negative rewards
              CaseKibbleDispenser* taxCase = new CaseKibbleDispenser(name,
  position, -taxAmount, this); boardCase = taxCase; if (DEBUG_CASE_LOADING) {
                  qDebug() << "Created Tax" << name << "at position" << position
  << "with amount" << taxAmount;
              }
              break;
          }
          default:
              qDebug() << "Unknown case type" << type << "at position" <<
  position; break;
      }
